...
function blockDelta(line) {
  ...
  for (const token of tokens) {
    if (token === "function" || token === "then" || token === "do" || token === "repeat" || token === "class" || token === "enum") {
      delta += 1;
...
function parseDocBlock(lines, lineIndex) {
  ...
  for (const line of collected) {
    const paramMatch = line.match(new RegExp(`^\\s*---\\s*@param\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+(${TYPE_NAME_RE})\\s*(.*)$`));
    if (paramMatch) {
...
function parseTypedDeclaration(line, declarationKind) {
  const match = line.match(/^\s*(?:var|local)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*:\s*(.+)$/);
  if (!match) {
    return null;
  }
...
function parseField(line) {
  ...
  if (match) {
    const parsed = parseTypedDeclaration(`local ${match[1]}: ${match[2]}`, "local");
    if (parsed) return parsed;
  }
...
function parseTypedLocal(line) {
  ...
  return { name: parsed.name, typeName: parsed.typeName, defaultExpr: parsed.defaultExpr };
}
...
function parseLocalVariable(line) {
  ...
  if (defaultExpr) {
    const ctorMatch = defaultExpr.match(/^new\s+([A-Za-z_][A-Za-z0-9_\.]*(?:<[^()]+>)?)\s*\(/);
    if (ctorMatch) {
...
function parseTypedParams(line) {
  // Capture optional `<T, U>` after method name and optional `: ReturnType`.
  const m = line.match(/^\s*(local\s+)?function\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s*<([^>]*)>)?\s*\((.*)\)\s*(?::\s*(.+))?$/);
  if (!m) {
    return null;
  }
  ...
  const genericParamsRaw = (m[3] || "").trim();
  const rawParams = (m[4] || "").trim();
  const returnTypeName = normalizeTypeName(m[5] || null);
...
function inferReceiverTypeSimple(receiverExpr, classInfo, methodParams, methodLocals) {
...
function inferForInVariableType(loopExpr, classInfo, methodParams, methodLocals) {
  ...
  const iterMatch = expr.match(/^([A-Za-z_][A-Za-z0-9_\.]*)\.iter\s*\(\s*\)\s*$/);
  if (iterMatch) {
    const receiverType = inferReceiverTypeSimple(iterMatch[1], classInfo, methodParams, methodLocals);
    if (receiverType) {
      const genericMatch = normalizeTypeName(receiverType).match(/^[A-Za-z_][A-Za-z0-9_\.]*<(.+)>$/);
      if (genericMatch) {
...
function buildModel(text) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const classMatch = lines[i].match(/^\s*class\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s*<([^>]*)>)?\s*(?:extends\s+(.+?)\s*)?$/);
    if (!classMatch) {
      continue;
    }
    ...
    const classTypeParams = classTypeParamsRaw
      ? classTypeParamsRaw.split(",").map((name) => name.trim()).filter(Boolean)
      : [];
        ...
        if (parsedMethod) {
          ...
          if (methodEnd >= 0) {
            for (let k = j + 1; k < methodEnd; k += 1) {
              const forInMatch = lines[k].match(/^\s*for\s+([A-Za-z_][A-Za-z0-9_]*)\s+in\s+(.+?)\s+do\s*$/);
              if (forInMatch) {
                const loopVarName = forInMatch[1];
                const loopExpr = forInMatch[2];
                const inferredLoopType = inferForInVariableType(loopExpr, classInfo, parsedMethod.typedParams, locals);
              ...
              if (localVariable) {
                locals.set(localVariable.name, {
                  ...
                  end: lines[k].indexOf(localVariable.name) + localVariable.name.length,
                });
              }

              const selfAssign = lines[k].match(/^\s*self\.([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.+)$/);
              if (selfAssign) {
                const fieldName = selfAssign[1];
                const expr = selfAssign[2].trim();

                let inferredType = null;
                const ctorMatch = expr.match(/^new\s+([A-Za-z_][A-Za-z0-9_\.]*)\s*\(/);
                if (ctorMatch) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    const inClassRange = classRanges.some((range) => i >= range.start && i <= range.finish);
    if (inClassRange) {
      continue;
    }
...
function getMethodAtLine(classInfo, line) {
  ...
  const overloadGroups = classInfo.methodOverloads ? classInfo.methodOverloads.values() : [];
...
function inferExpressionType(expr, model, classInfo, methodInfo, workspaceIndex) {
  const value = expr.trim();
  if (!value) {
    return null;
  }

  const ctorMatch = value.match(/^new\s+([A-Za-z_][A-Za-z0-9_\.]*(?:<[^()]+>)?)\s*\(/);
  if (ctorMatch) {
  ...
  if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(value)) {
    ...
    if (classInfo && classInfo.fields.has(value)) {
      ...
      const inferredType = classInfo.inferredFieldTypes && classInfo.inferredFieldTypes.get(value);
      if (inferredType && (fieldInfo.typeName === "any" || fieldInfo.typeName === "table" || fieldInfo.typeName === "nil")) {
        return inferredType;
      }
  ...
  if (chainParts.length > 1) {
    let currentType = inferExpressionType(chainParts[0], model, classInfo, methodInfo, workspaceIndex);
    for (let i = 1; i < chainParts.length; i += 1) {
      ...
      if (currentClass.fields.has(segment)) {
        ...
        const inferredType = currentClass.inferredFieldTypes && currentClass.inferredFieldTypes.get(segment);
        if (inferredType && (fieldInfo.typeName === "any" || fieldInfo.typeName === "table" || fieldInfo.typeName === "nil")) {
