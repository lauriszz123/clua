...
function makeHover(signature, docs) {
	const signatures = Array.isArray(signature)
	...
	const signatureBlock =
		signatures.length <= 1
			? `\`${signatures[0] || ""}\``
			: ["```clua", ...signatures, "```"].join("\n");

	return {
		contents: {
			kind: "markdown",
			value: docText ? `${signatureBlock}\n\n${docText}` : signatureBlock,
		},
	};
}
...
function getDisplayParams(methodInfo, docs) {
	...
	for (const param of typedParams) {
		...
		display.push({
			name: param.name,
			typeName:
				docParam && docParam.typeName ? docParam.typeName : param.typeName,
...
function buildMethodDisplayLabel(prefix, methodInfo, docs) {
	...
	const paramsText = displayParams
		.map((p) => `${p.name}: ${p.typeName}`)
		.join(", ");
	const returnSuffix =
		methodInfo && methodInfo.returnTypeName
...
function buildClassHoverData(classInfo) {
	...
	return {
		signature: buildMethodDisplayLabel(
			`new ${classInfo.name}`,
			ctor,
			ctor.docs,
		),
