...
const { buildModel, isKnownType, inferExpressionType } = require("./clua/vscode-clua-syntax/server/parser");
const { validateTextDocument } = require("./clua/vscode-clua-syntax/server/diagnostics");
...
function isEmpty(arr, message) {
  if (arr && arr.length > 0) {
    throw new Error((message || "") + ` (expected empty, got ${arr.length} items)`);
  }
}
...
`;
  ...
  const inferred = inferExpressionType("ball", model, mainClass, update, new Map());
...
`;
  const doc = { getText: () => src };
  const diags = validateTextDocument(doc, new Map());
  has(diags, (d) => d.message.includes("has no member 'update'"), "Should report unknown member update");
});
...
`;
  ...
  assert(update.locals.has("ball"), "Loop variable ball should be available in method locals");
  eq(update.locals.get("ball").typeName, "Ball", "Loop variable ball should infer Ball type");
